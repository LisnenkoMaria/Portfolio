...
import { getMatchesByCompetition } from '../api/footballApi';
...
export function LeagueCalendarPage() {
    ...
    useEffect(() => {
        async function loadMatches() {
            try {
                ...
                const data = await getMatchesByCompetition(
                    Number(id),
                    from || undefined,
                    to || undefined
                );
                ...
                if (data.length > 0 && data[0].competition) {
                    setLeagueName(data[0].competition.name);
                }
            } catch (err) {
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '1rem' }}>
            ...
            <h1 style={{ marginBottom: '0.5rem' }}>{leagueName || 'Календарь лиги'}</h1>
